...
def get_new_class_mutcd(class_name):
    if class_name == "stop":
        ...
        return 0  # mutcd
    return None  # discard anything else

def get_new_class_vienna(class_name):
    if class_name in ["prio_stop", "prio_give_way"]:
        return 2  # ambiguous — looks same as MUTCD yield/stop
...
with open("datasets/vienna/data.yaml") as f:
    vienna_classes = yaml.safe_load(f)['names']
with open("datasets/mutcd/data.yaml") as f:
    mutcd_classes = yaml.safe_load(f)['names']

# Remap all splits
for split in ["train", "valid", "test"]:
    remap_labels(f"datasets/vienna/{split}/labels", vienna_classes, get_new_class_vienna)
    remap_labels(f"datasets/mutcd/{split}/labels", mutcd_classes, get_new_class_mutcd)
