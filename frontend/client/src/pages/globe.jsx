...
function topCountryButtonColors(index, total) {
    const hue = Math.round(topCountryRankHue(index, total) * 360);
    return {
        border: `1px solid hsl(${hue}, 78%, 33%)`,
        background: `hsl(${hue}, 78%, 47%)`,
        color: '#fff',
    };
}
...
function Globe({ cord = null }) {
    ...
    const [locationLabel, setLocationLabel] = useState('Choose a section to focus the globe.');
    const [candidateIndex, setCandidateIndex] = useState(-1);
    const [isPaused, setIsPaused] = useState(false);

    // Only use points extracted from cord – no external points
    ...
    const onGoToSafe = () => {
        const { lat, lon } = readLatLon(safePoint?.coordinate);
        if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
            setLocationLabel('Safe geolocalization not available.');
            return;
        }
        focusLocationRef.current?.(lat, lon);
        setLocationLabel(`Safe geolocalization: ${nearestCityLabel(lat, lon)}`);
    };
    ...
    const onGoToCountry = (country) => {
        ...
        setLocationLabel(`Top country: ${country.name}`);
    };
    ...
    useEffect(() => {
        ...
        topCountryCenters.forEach((center, index) => {
            ...
            const color = new THREE.Color().setHSL(
                topCountryRankHue(index, topCountryCenters.length),
                0.82,
                0.55,
            );
    ...
    }, [cord]); // Only depend on cord, not points
